...
MTS_PY_EXPORT(VolumeGrid) {
    ...
    volumegrid.def(py::init([](py::array obj, bool compute_max=true) {
            ...
            ScalarVector3i size(obj.shape()[0], obj.shape()[1], obj.shape()[2]);
        ...
        .def_property_readonly("__array_interface__", [](VolumeGrid &grid) -> py::object {
            ...
            if (grid.channel_count() == 1)
                result["shape"] = py::make_tuple(size.x(), size.y(), size.z());
            else
                result["shape"] = py::make_tuple(size.x(), size.y(), size.z(), grid.channel_count());
